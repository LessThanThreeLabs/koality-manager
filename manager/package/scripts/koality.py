...
class PlatformPackageScript(ShellScript):
	...
	def get_script(cls):
		return cls.multiline(
			'cd /tmp',
			'git clone git@github.com:LessThanThreeLabs/agles.git -b 0.7.2',
...
class WebPackageScript(ShellScript):
	...
	def get_script(cls):
		return cls.multiline(
			...
			'rm -rf webserver',
			'wget https://s3.amazonaws.com/koality_code/libraries/private-cd855575be99a357/koality-webserver-0.7.2.tgz',
			'tar -xvf koality-webserver-0.7.2.tgz',
			'rm koality-webserver-0.7.2.tgz',
